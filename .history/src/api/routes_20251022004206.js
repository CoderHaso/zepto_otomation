...
module.exports = (app) => {
  ...
  // ZeptoMail - Template-Based Email Send (Axios ile)
  app.post('/api/zeptomail/send-template', async (req, res) => {
    try {
      ...
      const results = [];
      const apiUrl = `https://${host || 'api.zeptomail.com'}/v1.1/email/template`;
      ...
      for (const contact of contacts) {
        try {
          ...
          console.error(`✗ Failed to send to ${contact.email}:`, error.response?.data || error.message);
          results.push({ 
            contact: contact.email, 
            status: 'failed', 
  ...
  app.post('/api/zeptomail/test-send', async (req, res) => {
    try {
      ...
      const apiUrl = `https://${settings.host || 'api.zeptomail.com'}/v1.1/email/template`;
      ...
      console.log('✓ Mail gönderildi!', response.data);
  ...
  // ZeptoMail - Template Listeleme (OAuth gerekli)
  app.post('/api/zeptomail/list-templates', async (req, res) => {
    try {
      const { oauthToken, mailAgent, host } = req.body;
      
      console.log('=== List Templates ===');
      console.log('Mail Agent:', mailAgent);
      
      if (!oauthToken) {
        return res.status(400).json({ error: 'OAuth token gerekli' });
      }
      ...
      const apiUrl = `https://${host || 'api.zeptomail.com'}/v1.1/mailagents/${mailAgent}/templates`;
      ...
      console.log('✓ Templates loaded:', response.data.data?.length || 0);
  ...
  // ZeptoMail - Template Detay (OAuth gerekli)
  app.post('/api/zeptomail/get-template', async (req, res) => {
    try {
      const { oauthToken, mailAgent, templateKey, host } = req.body;
      
      console.log('=== Get Template ===');
      console.log('Template Key:', templateKey);
      
      if (!oauthToken || !mailAgent || !templateKey) {
        return res.status(400).json({ error: 'OAuth token, Mail Agent ve Template Key gerekli' });
      }
      
      const apiUrl = `https://${host || 'api.zeptomail.com'}/v1.1/mailagents/${mailAgent}/templates/${templateKey}`;
      ...
      console.log('✓ Template loaded:', response.data.data?.template_name);
  ...
  // ZeptoMail - Template Güncelleme (OAuth gerekli)
  app.post('/api/zeptomail/update-template', async (req, res) => {
    try {
      const { oauthToken, mailAgent, templateKey, templateData, host } = req.body;
      
      console.log('=== Update Template ===');
      console.log('Template Key:', templateKey);
      
      if (!oauthToken || !mailAgent || !templateKey || !templateData) {
        return res.status(400).json({ error: 'Tüm alanlar gerekli' });
      }
      
      const apiUrl = `https://${host || 'api.zeptomail.com'}/v1.1/mailagents/${mailAgent}/templates/${templateKey}`;
      
      const response = await axios.put(apiUrl, templateData, {
        headers: {
          'Authorization': `Zoho-oauthtoken ${oauthToken}`,
          'Content-Type': 'application/json',
          'Accept': 'application/json'
