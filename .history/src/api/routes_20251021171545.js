  ...
  app.post('/api/zeptomail/test', async (req, res) => {
    try {
      ...
      if (!apiKey || !apiKey.startsWith('Zoho-enczapikey')) {
        return res.status(400).json({ error: 'API key formatı yanlış' });
      }
      ...
      const response = await axios.get(`https://api.zeptomail.com/v1.1/mailagents/${mailAgent}/templates?offset=0&limit=1`, {
        headers: { 
      ...
      res.json({ success: true, message: 'API bağlantısı başarılı!' });
    } catch (error) {
  ...
  app.post('/api/zeptomail/sync-templates', async (req, res) => {
    try {
      ...
      zeptoTemplates.forEach(zt => {
        const exists = localTemplates.find(lt => lt.zeptomailId === zt.template_key);
        if (!exists) {
          localTemplates.push({
            id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
