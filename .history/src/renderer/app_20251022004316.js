    ...
    if (this.templates.length === 0) {
      container.innerHTML = `
        <div class="template-item">
          ...
          <p>Template yüklemek için "🔄 Yenile" butonuna tıklayın.</p>
        </div>
      `;
      return;
    }
      ...
      return `
        <div class="template-item">
          ...
          <div style="margin-top: 0.5rem; display: flex; gap: 0.5rem;">
            ${isZeptoTemplate ? `<button onclick="app.viewTemplate('${template.zeptomailTemplateKey || template.zeptomailTemplateAlias}')">👁️ Görüntüle</button>` : ''}
            ${isZeptoTemplate ? `<button onclick="app.editZeptoTemplate('${template.zeptomailTemplateKey || template.zeptomailTemplateAlias}')">✏️ Düzenle</button>` : `<button onclick="app.editTemplate('${template.id}')">✏️ Düzenle</button>`}
