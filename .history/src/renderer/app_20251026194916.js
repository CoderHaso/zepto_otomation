...
class ZeptoMailDomainManager {
  ...
  updateAccountAssignments() {
    ...
    container.innerHTML = `
      <div class="form-group">
        <label>Hangi account'lardan gönderilsin? (Her account ${this.selectedContacts.size} kişiye gönderecek)</label>
        <p class="hint">✨ Her seçili account, TÜM seçili kişilere gönderim yapacak</p>
  ...
  async sendNow() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const totalExpected = selectedAccountIds.length * this.selectedContacts.size;
        
        this.showMessage(`✓ Gönderim tamamlandı! ${successful} başarılı, ${failed} başarısız (Toplam: ${totalExpected})`, 'success');
