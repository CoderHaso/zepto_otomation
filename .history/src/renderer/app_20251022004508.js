...
class YazarIletisimPaneli {
    ...
    if (this.templates.length === 0) {
      container.innerHTML = `
        <div class="template-item">
          ...
          <p>"📥 Template Ekle" butonuna tıklayarak ZeptoMail'deki template key'ini girin.</p>
        </div>
      `;
      return;
    }
    ...
    container.innerHTML = this.templates.map(template => {
      ...
      return `
        <div class="template-item">
          ...
          ${isZeptoTemplate ? `<p><strong>Template Key:</strong> <code style="background: #f5f5f5; padding: 0.2rem 0.5rem; border-radius: 3px; font-size: 0.85rem;">${template.zeptomailTemplateKey || template.zeptomailTemplateAlias}</code></p>` : ''}
          <p style="color: #666; font-size: 0.9rem; margin-top: 0.5rem;">
            ${isZeptoTemplate ? '📝 ZeptoMail web panelinde düzenleyin: <a href="https://www.zoho.com/zeptomail/" target="_blank">ZeptoMail Console</a>' : 'Yerel olarak oluşturuldu'}
          </p>
          <div style="margin-top: 0.5rem; display: flex; gap: 0.5rem;">
            ${!isZeptoTemplate ? `<button onclick="app.editTemplate('${template.id}')">✏️ Düzenle</button>` : ''}
            <button onclick="app.deleteTemplate('${template.id}')" style="background: #e74c3c; color: white;">🗑️ Listeden Kaldır</button>
          </div>
        </div>
      `;
    }).join('');
  ...
  // ZeptoMail Template Actions
  async refreshTemplatesFromZepto() {
    if (!this.settings.oauthToken) {
      this.showMessage('OAuth Token gerekli! Ayarlar → OAuth Token alanını doldurun.', 'error');
      return;
    }

    try {
      const response = await fetch('http://localhost:3000/api/zeptomail/list-templates', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          oauthToken: this.settings.oauthToken,
      ...
      if (!response.ok) {
        ...
        this.showMessage(error.error || 'Template listesi alınamadı!', 'error');
        return;
      }

      const result = await response.json();
      const zeptoTemplates = result.data || [];

      // Yerel template listesine ekle
      const templatesPath = 'http://localhost:3000/api/templates';
      
      for (const zt of zeptoTemplates) {
        // Kontrol et, zaten var mı?
        const exists = this.templates.find(t => t.zeptomailTemplateKey === zt.template_key);
        if (!exists) {
          await fetch(templatesPath, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
      ...
      await this.loadTemplates();
      this.showMessage(`✓ ${zeptoTemplates.length} template yüklendi!`, 'success');

    } catch (error) {
      console.error('Refresh templates error:', error);
      this.showMessage('Template listesi yüklenemedi!', 'error');
    }
  }

  async viewTemplate(templateKey) {
    if (!this.settings.oauthToken) {
      this.showMessage('OAuth Token gerekli!', 'error');
      return;
    }

    try {
      const response = await fetch('http://localhost:3000/api/zeptomail/get-template', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          oauthToken: this.settings.oauthToken,
      ...
      if (!response.ok) {
        const error = await response.json();
        this.showMessage(error.error || 'Template detayı alınamadı!', 'error');
        return;
      }
      ...
      modalBody.innerHTML = `
        ...
        <div class="form-group">
          ...
          <div style="background: #f5f5f5; padding: 1rem; border-radius: 4px; max-height: 300px; overflow-y: auto;">
            <pre style="margin: 0; white-space: pre-wrap;">${template.htmlbody || '-'}</pre>
          </div>
        </div>
        <div class="form-group">
          <label>Text İçerik:</label>
          <div style="background: #f5f5f5; padding: 1rem; border-radius: 4px; max-height: 200px; overflow-y: auto;">
            <pre style="margin: 0; white-space: pre-wrap;">${template.textbody || '-'}</pre>
          </div>
        </div>
        <div style="text-align: right;">
          <button onclick="app.hideModal()">Kapat</button>
        </div>
      `;
      this.showModal();
      ...
      this.showMessage('Template görüntülenemedi!', 'error');
    }
  }

  async editZeptoTemplate(templateKey) {
    if (!this.settings.oauthToken) {
      this.showMessage('OAuth Token gerekli!', 'error');
      return;
    }
    ...
    try {
      ...
      const modalBody = document.getElementById('modal-body');
      modalBody.innerHTML = `
        <h2>✏️ Template Düzenle</h2>
        <form id="zepto-edit-form">
          <div class="form-group">
            <label>Template Adı:</label>
            <input type="text" id="edit-template-name" value="${template.template_name}" required>
          </div>
          <div class="form-group">
            <label>Konu:</label>
            <input type="text" id="edit-template-subject" value="${template.subject || ''}" required>
          </div>
          <div class="form-group">
            <label>HTML İçerik:</label>
            <textarea id="edit-template-html" rows="10">${template.htmlbody || ''}</textarea>
          </div>
          <div class="form-group">
            <label>Text İçerik:</label>
            <textarea id="edit-template-text" rows="5">${template.textbody || ''}</textarea>
          </div>
          <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
            <button type="button" onclick="app.hideModal()">İptal</button>
      ...
      document.getElementById('zepto-edit-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        await this.saveZeptoTemplate(templateKey);
      });
      ...
      this.showMessage('Template düzenlenemedi!', 'error');
    }
  }

  async saveZeptoTemplate(templateKey) {
    const templateData = {
      ...
      htmlbody: document.getElementById('edit-template-html').value,
      textbody: document.getElementById('edit-template-text').value
    };

    try {
      const response = await fetch('http://localhost:3000/api/zeptomail/update-template', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          oauthToken: this.settings.oauthToken,
      ...
      if (!response.ok) {
        const error = await response.json();
        this.showMessage(error.error || 'Template güncellenemedi!', 'error');
        return;
      }

      this.hideModal();
      this.showMessage('✓ Template güncellendi!', 'success');
